...
import { BatchToastManager } from '@/lib/utils/batch-toast';
import { useModalConfirmation } from '@/hooks/useModalConfirmation';
import { DOCUMENT_CONFIG } from '@/constants/documents';
import {
  createVirtualExports,
  parseVirtualUrls,
  parseRealUrls,
  filterDuplicateDocuments,
  type PreparedDocument,
  type ExportType
} from '@/lib/utils/document-preparation';
...
function IhaleTakipPageInner() {
  ...
  // 🆕 Modal confirmation hook - Seçim varsa kapatma onayı iste
  const hasUnsavedChanges = selectedDocuments.length > 0 || preparedDocuments.length > 0;
  const confirmModalClose = useModalConfirmation(hasUnsavedChanges, {
    message: `${selectedDocuments.length} döküman seçili${preparedDocuments.length > 0 ? ` ve ${preparedDocuments.length} döküman hazırlanmış` : ''}. Modal kapatılsın mı?`
  });
  ...
  // 🆕 Dökümanları hazırla ve önizleme göster (REFACTORED with utilities)
  const prepareDocuments = async () => {
    console.log('📦 prepareDocuments başlatıldı:', {
      seciliDokumanSayisi: selectedDocuments.length,
      ihaleBilgisi: selectedTender ? `${selectedTender.title.substring(0, 40)}...` : 'Yok',
      fullContentVarMi: !!fullContent
    });

    // Validation
    if (!selectedTender || !fullContent) {
      console.error('❌ Validation hatası: İhale detayı veya içerik eksik');
      toast.error('⚠️ İhale detayı yüklenmemiş');
      return;
    }

    if (!fullContent.fullText || fullContent.fullText.length === 0) {
      console.error('❌ Validation hatası: İhale metni boş');
    ...
    try {
      ...
      if (selectedDocuments.length > 0) {
        ...
        toastManager.start(selectedDocuments.length, 'Dökümanlar hazırlanıyor');

        // 🎯 URL parsing with utility functions
        const virtualExportTypes = parseVirtualUrls(selectedDocuments);
        const realUrls = parseRealUrls(selectedDocuments);

        console.log(`🔍 URL analizi:`, {
          toplamSecim: selectedDocuments.length,
          virtualExport: virtualExportTypes.length,
          realDokuman: realUrls.length
        });

        const allPrepared: PreparedDocument[] = [];

        // 🎯 Virtual exports - using utility function
        if (virtualExportTypes.length > 0) {
          console.log(`📝 ${virtualExportTypes.length} virtual export oluşturuluyor...`);

          try {
            const virtualDocs = createVirtualExports(
              { title: selectedTender.title, organization: selectedTender.organization },
              {
                details: fullContent.details,
            ...
            allPrepared.push(...virtualDocs);

            console.log(`✅ ${virtualDocs.length} virtual export oluşturuldu`);

            // Update progress for each virtual file
            virtualDocs.forEach((doc, idx) => {
              toastManager.update(idx + 1, selectedDocuments.length, doc.title);
            });
          } catch (error: any) {
            console.error('❌ Virtual export hatası:', error);
            toast.error('⚠️ Virtual dosyalar oluşturulamadı', {
              description: error.message
            });
          }
        }

        // 🎯 Real documents - using existing downloader
        if (realUrls.length > 0) {
          console.log(`📥 ${realUrls.length} gerçek döküman indiriliyor...`);

          try {
            const downloadedFiles = await downloadDocuments(realUrls, {
              onProgress: (progress) => {
                toastManager.update(
                  progress.current + virtualExportTypes.length,
            ...
            console.log(`📦 ${downloadedFiles.length} dosya indirildi (bazıları ZIP'ten extract olmuş olabilir)`);

            // Convert to PreparedDocument format
            const realPrepared: PreparedDocument[] = downloadedFiles.map(df => ({
              title: df.title,
              url: df.url,
              ...
              size: df.size,
              type: 'document',
              isFromZip: df.isFromZip || false,
              originalFilename: df.originalFilename
            }));

            allPrepared.push(...realPrepared);

            console.log(`✅ ${realPrepared.length} gerçek döküman hazırlandı`);
          } catch (error: any) {
            console.error('❌ Döküman indirme hatası:', error);
            toast.error('⚠️ Bazı dökümanlar indirilemedi', {
              description: error.message
            });
          }
        }

        // 🎯 Duplicate filtering - using utility function
        console.log(`🔍 Duplicate kontrolü yapılıyor: ${allPrepared.length} dosya kontrol edilecek`);

        setPreparedDocuments(prev => {
          const uniqueFiles = filterDuplicateDocuments(allPrepared, prev);

          if (uniqueFiles.length < allPrepared.length) {
            const skippedCount = allPrepared.length - uniqueFiles.length;
            console.warn(`⚠️ ${skippedCount} duplicate dosya atlandı`);
            toast.warning(`⚠️ ${skippedCount} dosya zaten ekliydi, atlandı`);
          }

          console.log(`📦 State güncelleniyor: ${uniqueFiles.length} yeni dosya ekleniyor`);
          return [...prev, ...uniqueFiles];
        });

        toastManager.complete(allPrepared.length, 'Tüm dökümanlar hazır!');
        console.log(`🎉 prepareDocuments başarıyla tamamlandı: ${allPrepared.length} dosya`);
      } else {
        console.warn('⚠️ Seçili döküman yok');
        toast.info('ℹ️ Döküman seçilmedi');
      }
      ...
      console.error('❌ prepareDocuments kritik hatası:', {
        hata: error.message,
        stack: error.stack?.substring(0, 200)
      });
      toastManager.error('Dökümanlar hazırlanırken hata oluştu', error.message);
      setIsAnalyzing(false);
      setLoadingStartTime(null);
    }
  };
...
Lütfen tarayıcı konsolunu kontrol edin ve geliştiriciyle iletişime geçin.`;
  ...
  // 🆕 Modal'ı kapat ve URL parametresini temizle (confirmation ile)
  const closeModal = () => {
    // 🎯 Confirmation wrapper - Seçim varsa onay iste
    confirmModalClose(() => {
      ...
      setPreparedDocuments([]); // 🆕 Hazırlanan dökümanları da temizle
                          ...
                          <div className="p-6 space-y-4 bg-gradient-to-br from-gray-50 to-white">
                            ...
                            <div className="space-y-3">
                              {(() => {
                                ...
                                const totalPages = Math.ceil(totalDocs / DOCUMENT_CONFIG.PAGINATION.ITEMS_PER_PAGE);
                                const startIdx = (docPage - 1) * DOCUMENT_CONFIG.PAGINATION.ITEMS_PER_PAGE;
                                const endIdx = Math.min(startIdx + DOCUMENT_CONFIG.PAGINATION.ITEMS_PER_PAGE, totalDocs);
                                const visibleDocs = fullContent.documents.slice(startIdx, endIdx);

                                return (
                                  <>
                                    {/* Pagination Info */}
                                    {totalDocs > DOCUMENT_CONFIG.PAGINATION.ITEMS_PER_PAGE && (
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {isSelected && (
                                      <div className={`absolute -inset-1 bg-gradient-to-r ${docInfo.gradient} opacity-30 blur-lg rounded-3xl transition-opacity duration-150`}></div>
                                    )}

                                    <div
                                      className={`relative overflow-hidden rounded-2xl border-2 transition-colors duration-150 cursor-pointer backdrop-blur-sm ${
                                        isSelected
                                          ? `bg-white/90 ${docInfo.borderColor} shadow-lg`
                                          : 'bg-white/80 border-gray-200/50 hover:border-gray-300/70 hover:bg-white/95'
                                      }`}
                                    ...
                                    <div className="relative flex items-center gap-4 p-5">
                                      ...
                                      <div className="flex-shrink-0">
                                        <button
                                          ...
                                          className={`relative w-14 h-7 rounded-full transition-colors duration-150 ${
                                            isSelected
                                              ? `bg-gradient-to-r ${docInfo.gradient}`
                                              : 'bg-gray-300 hover:bg-gray-400'
                                          }`}
                                          aria-label="Toggle selection"
                                        >
                                          <div
                                            className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow-md transition-transform duration-150 flex items-center justify-center ${
                                      ...
                                      <div className={`flex-shrink-0 w-14 h-14 rounded-xl ${docInfo.iconBg} flex items-center justify-center text-2xl shadow-sm`}>
                                        {docInfo.icon}
                                      </div>
